...
if [[ "${image_kind}" != "raw" && "${image_kind}" != "package" ]]; then
  echo "Image kind must be raw or package" >&2
  exit 2
fi
extract_sha256="${sha256}"
[[ "${image_kind}" == "package" ]] && extract_sha256=none
chunk_dir="$(mktemp -d)"
...
curl --fail-with-body --location --retry 5 --retry-all-errors --retry-delay 2 \
  ...
  --header "X-OpenHD-Extract-Sha256: ${extract_sha256}" \
...
curl --fail-with-body --location --retry 5 --retry-all-errors --retry-delay 2 \
  ...
  "https://openhd.tech/api/internal/dev-images/${GITHUB_RUN_ID}/promote"
