...
usage="Usage: build_luckfox_update.sh <ohd-seed-dir> <openhd.tar.gz> <openhd-sys-utils.tar.gz> <output-dir>"
...
for manifest, component in expected:
    ...
    assert "luckfox" in manifest["platform"].lower()
    assert manifest["architecture"] in ("arm", "aarch64")
    assert manifest["sdk_sha256"] != "unknown"
assert openhd["sdk_sha256"] == sysutils["sdk_sha256"], "component SDK checksums differ"
assert openhd["sdk_buildroot_commit"] == sysutils["sdk_buildroot_commit"], "commits differ"
PY
...
file "${stage_dir}/usr/bin/openhd" | grep -q 'ARM'
file "${stage_dir}/usr/bin/openhd_sys_utils" | grep -q 'ARM'
...
# image to the full partition size guarantees that old UBI headers are erased.
# Assuming same size as X21 (100MB) for now
ohd_partition_size=$((0x6400000))
ohd_image_size="$(stat -c '%s' "${update_work}/ohd.img")"
if ((ohd_image_size > ohd_partition_size)); then
  echo "OHD UBI image exceeds partition: ${ohd_image_size} > ${ohd_partition_size}" >&2
  exit 1
fi
...
software =
{
    ...
    description = "OpenHD Luckfox OHD partition update";
    images: (
        {
            filename = "ohd.img";
            device = "mtd9";
...
versioned_name="OpenHD-Luckfox-${openhd_version}-${sysutils_version}.ohd"
latest_name="OpenHD-Luckfox-latest.ohd"
(
  cd "${update_work}"
  ...
  cat >"${output_dir}/${update_name}.manifest.json" <<EOF
{
  ...
  "ohd_seed_sha256": "${LUCKFOX_OHD_SEED_SHA256:-unknown}",
...
cat >"${output_dir}/openhd-luckfox-updates.json" <<EOF
{
  "os_list": [
    {
      "name": "OpenHD Luckfox",
      "description": "SWUpdate package for an existing Luckfox installation",
      "icon": "https://fra1.digitaloceanspaces.com/openhd-images/Downloader/OpenHD-advanced.png",
      "subitems": [
        {
          "name": "OpenHD Luckfox latest",
