...
log "We now define the size to be ~15GB (the maximum size we have in our github builder, this doesn't affect the output image because we're resizing it in the end before uploading the image)" 
FILESIZE=$(stat -c%s "IMAGE.img")
DIFFERENCE=$((WANTEDSIZE - FILESIZE - 1))
...
if [[ "${OS}" != "ubuntu-x86" && "${OS}" != "debian-X20" ]]; then
    ...
    if [[ "${OS}" == "radxa-debian-rock5a" || "${OS}" == "radxa-debian-rock5b" || "${OS}" == "radxa-debian-rock-cm3" || "${OS}" == "radxa-debian-rock-cm3-core3566" ]]; then
        ...
        ROOT_OFFSET=$(echo "$PARTED_OUT" | grep -e "^ ${ROOT_PART}" | xargs echo -n | cut -d" " -f 2 | tr -d s)

        echo "ROOT PART: ${ROOT_PART}"
        echo "ROOT OFFSET: $ROOT_OFFSET"
        echo "IF EDITING THIS SCRIPT THE SPACES MATTER FOR FDISK COMMANDS"
