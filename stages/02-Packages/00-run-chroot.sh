...
#!/bin/bash
 ...
 if [[ "${OS}" == "raspbian" ]]; then
    # Remove bad and unnecessary symlinks if system is not ubuntu
     ...
     PLATFORM_PACKAGES="open-hd-web-ui openhd-userland openhd-linux-pi openhd-linux-pi-headers libsdl2-dev libspdlog-dev libcamera-openhd libcamera-apps-openhd libavcodec-dev libavformat-dev gstreamer1.0-gl gst-plugins-good gst-openhd-plugins openhd-qt openhd qopenhd libsodium-dev libpcap-dev git nano openssh-server libboost-filesystem1.74-dev meson"
 fi

 if [[ "${OS}" == "ubuntu-x86" ]] ; then
         echo "OS is ubuntu, we're building for x86"
         sudo apt-mark hold grub-efi-amd64-signed linux-generic linux-headers-generic linux-image-generic linux-generic-hwe-22.04 linux-image-generic-hwe-22.04 linux-headers-generic-hwe-22.04
         ...
         echo grub-efi-amd64-signed | sudo dpkg --set-selections     
         sudo apt update
         sudo apt upgrade
        ...
        #install all qt-dependencies (needs to be cleaned in the future)
            ...
            apt -y install libfuse2 mono-complete mono-runtime libmono-system-windows-forms4.0-cil libmono-system-core4.0-cil libmono-system-management4.0-cil libmono-system-xml-linq4.0-cil
            apt -y install libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev gstreamer1.0-plugins-base gstreamer1.0-plugins-good gstreamer1.0-plugins-ugly gstreamer1.0-plugins-bad libgstreamer-plugins-bad1.0-dev gstreamer1.0-pulseaudio gstreamer1.0-tools gstreamer1.0-alsa gstreamer1.0-qt5
        mkdir -p /opt/X86/
