...
function install_raspbian_packages {
    ...
    PLATFORM_PACKAGES="apt-transport-https git curl apt-utils firmware-atheros firmware-misc-nonfree open-hd-web-ui openhd-userland openhd-linux-pi openhd-linux-pi-headers libsdl2-dev libspdlog-dev libcamera-openhd libcamera-apps-openhd libavcodec-dev libavformat-dev mavsdk gstreamer1.0-gl gst-latest openhd-qt openhd qopenhd libsodium-dev libpcap-dev git nano openssh-server libboost-filesystem1.74-dev meson"
}
...
function install_debian_packages {
    PLATFORM_PACKAGES="apt-transport-https git curl apt-utils openhd qopenhd rtl8812au-autocompiler procps git cmake dkms"
}
...
function install_jetson_packages {
    ...
    PLATFORM_PACKAGES="apt-transport-https git curl apt-utils nano mingetty libgstreamer-plugins-base1.0-dev python-pip libelf-dev libboost1.74-dev openhd-linux-jetson openhd"
}
